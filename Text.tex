...
If $A$ is measurable, then we write $\mu(A)$ instead of $\mu^*(A)$. We shall $\mu(A)$ the \emph{Lebesgue measure}.
...
Recall that a set $A \subset \mathbb{R}$ is \emph{meager} if $A$ is the union of countable of nowhere dense sets.

\begin{exercise}
  The set of all $A \subset \mathbb{R}$ meager sets is a $\sigma$-ideal.
\end{exercise}
