...
Let us introduce the \emph{arrow notation}. Let $\lambda$ and $\kappa$ be infinite cardinals, let $m$ be a cardinal
and let $n < \omega$. The notation $\kappa \to (\lambda)^n_{m}$ (read as ``$\kappa$ arrows $\lambda$'') stands for
the following \emph{partition property}: every partition of $[\kappa]^n$ into $m$ pieces has a homogeneous set
of size $\lambda$. That is, every function $F : [\kappa]^n \to m$ is constant on $[H]^n$ for some $H \subset \kappa$
such that $|H|= \lambda$. By using this notation, one can reword Ramsey's theorem the following way:

\begin{center}
  $\forall n, k < \omega \:\: \aleph_0 \to (\aleph_0)^n_k$.
\end{center}
One often drops the subscript $m$ when $m = 2$, so we have
\begin{center}
  $\kappa \to (\lambda)^n \leftrightharpoons \kappa \to (\lambda)^n_2$.
\end{center}

The relation $\kappa \to \lambda$ is true of $\kappa$ is made larger or if $\lambda$ or $m$ are made smaller.
The relation is the case if only $\kappa \geq \lambda$ and $k > m$ and it is false for $k = m$.
So we can always assume $2 \geq m < \kappa$ and $\lambda \leq \kappa$. 
When $n = 1$, then the relation holds whenever either $\kappa > \lambda$ or $\kappa = \lambda$ and $\cf \kappa > m$.
By default, we shall assume that $n \geq 2$.
...
\begin{lemma}
  ...
  \begin{center}
    $2^{\kappa} \not\to (\omega)^2_{\kappa}$.
  \end{center}
  That is, there is a partition of $2^{\kappa}$ into $\kappa$ pieces such that it does not have
  an infinite homogeneous set.
\end{lemma}

\begin{proof}
  One can find a partition with no homogeneous set of size $3$.
  Let $S = 2^{\kappa}$ and let $F : [S]^2 \to \kappa$
  defined as
  \begin{center}
    $F(\{ f, g\}) = \text{the least $\alpha < \kappa$ such that $f(\alpha) \neq g(\alpha)$}$.
  \end{center}
  If $f, g, h$ are distinct elements of $S$, then we have $F(\{ f, g\}) = F(\{ f, h\}) = F(\{ g, h\})$.
\end{proof}

The following is a rather direct generalisation of Ramsey's theorem:
\begin{lemma}
  For every $\kappa$,
  \begin{center}
    $2^{\kappa} \not\kappa (\kappa^+)^2$.
...
\begin{proof}
  Let $2^{\kappa} = \lambda$ and let $\{ f_{\alpha} \: | \: \alpha < \kappa \}$
  be an enumeration of the set $P = \{ 0, 1\}^{\kappa}$. Let $\prec$ be a linear ordering of $\lambda$ induced
  by the lexicographic ordering of $P$:
  \begin{center}
    $\alpha \prec \beta \leftrightarrow f_{\alpha} \prec f_{\beta}$.
  \end{center}

  Define a partition $F : [\lambda]^2 \to \{ 0, 1 \}$ by letting $F(\{ \alpha, \beta \}) = 1$
  when the ordering on $\{ \alpha, \beta \}$ agrees with the natural ordering and $F(\{ \alpha, \beta \}) = 0$
  for otherwise. If $H \subset \lambda$ is a homogeneous set of order-type $\kappa^+$.
  $\{ f_{\alpha} \: | \: \alpha \in H \}$ constitutes a decreasing (or increasing) $\kappa^+$-sequence, which contradicts
  the below lemma.
\end{proof}
...
\begin{proof} Let us consider the increasing case. The decreasing case is considered similarly.
  Assume that $W = \{ f_{\alpha} \: | \: \alpha < \kappa^+\} \subset \{ 0, 1\}^{\kappa}$ is such that
  $f_{\alpha} < f_{\beta}$ whenever $\alpha < \beta$. Let $\gamma \leq \kappa$ be the least $\gamma$
  such that the set $\{ f_{\alpha} \upharpoonright \gamma \: | \: \alpha < \kappa^+ \}$ has size $\kappa^+$.
  Let $Z \subset W$ be such that $|Z| = \kappa^+$ and $f \upharpoonright \gamma = g \upharpoonright \gamma$
  for $f, g \in Z$. We may assume that $Z = W$.

  For each $\alpha < \kappa^+$, let $\xi_{\alpha}$ such that 
  $f_{\alpha} \upharpoonright \xi_{\alpha} = f_{\alpha + 1} \upharpoonright \xi_{\alpha}$ and 
  $f_{\alpha}(\xi_{\alpha}) = 0$, $f_{\alpha + 1}(\xi_{\alpha}) = 1$, so clearly $\xi_{\alpha} < \gamma$.
  Thus there exists $\xi < \gamma$ such that $\xi = \xi_{\alpha}$ for $\kappa^+$ elements $f_{\alpha}$
  of $W$.
  If $\xi = \xi_{\alpha} = \xi_{\beta}$ and 
  $f_{\alpha} \upharpoonright \xi = f_{\beta} \upharpoonright \xi$,
  then $f_{\beta} < f_{\alpha + 1}$ and $f_{\alpha} < f_{\beta + 1}$, so $f_{\alpha} = f_{\beta}$.
  The set $\{ f_{\alpha} \upharpoonright \: | \: \alpha < \kappa^+ \}$ has size $\kappa^+$, but we have already assume that
  $\gamma$ is minimal.
\end{proof}

\begin{theorem}[Erd\"{o}s-Rado]
$ $

  \begin{center}
    $\beth^+_n \to (\aleph_1)^{n + 1}_{\aleph_0}$.
  \end{center}

  In particular, $(2^{\aleph_0})^+ \to (\aleph_1)^2_{\aleph_0}$.
\end{theorem}

\begin{proof}
  Let us first show the case $n = 1$.
  Let $\kappa = (2^{\aleph_0})^+$ and let $F : [\kappa]^2 \to \omega$ be a partition of $[\kappa]^2$ into 
  $\aleph_0$ pieces. Let us find a homogeneous $H \subset \kappa$ of size $\aleph_1$.

  Let $a \in \kappa$, let $F_{a}$ be a function on $\kappa - \{ a \}$
  defined by $F_a(x) = F(\{ a, x\})$.
  Let us first show the following claim: there exists a set $A \subset \kappa$
  such that $|A| = 2^{\aleph_0}$ such that for every comfortable $C \subset A$
  and every $u \in \kappa - C$ there exists $v \in A - C$ such that $F_v$ agrees with $F_u$ on $C$.

  To show the claim, construct an $\omega_1$-sequence
  \begin{center}
    $A_0 \subset A_1 \subset \ldots \subset A_{\alpha} \cdots$ for $\alpha < \omega_1$.
  \end{center}
  of subsets of $\kappa$, each of which is of size $2^{\aleph_0}$.
  Let $A_0$ be arbitrary and for each limit $\alpha$ put
  \begin{center}
    $A_{\alpha} = \bigcup \limits_{\beta < \alpha} A_{\beta}$.
  \end{center}

  Given $A_{\alpha}$, there exists a set $A_{\alpha + 1} \supset A_{\alpha}$ of size $2^{\aleph_0}$
  such that for each countable $C \subset A_{\alpha}$ and every $u \in \kappa - C$
  there exists $v \in A_{\alpha + 1} - C$ such that $F_v$ agrees with $F_u$ on $C$. Thus let
  \begin{center}
    $A = \bigcup \limits_{\alpha < \omega_1} A_{\alpha}$
  \end{center}
  and $A$ clearly satisfies the required property. Choose $a \in \kappa - A$ and
  construct a sequence $\langle x_{\alpha} \: : \: \alpha < \omega_1 \rangle$ in $A$ the following way.
  Let $x_0 \in A$ be arbitrary. Given $\{ x_{\beta} \: | \: \beta < \alpha \} = C$,
  let $x_{\alpha}$ be some $v \in A - C$ such that $F_v$ agrees with $F_{\alpha}$ on $C$.
  Let $X = \{ x_{\alpha} \: | \: \alpha < \omega_1 \}$.

  Now let us consider the function $G : X \to \omega$ defined by $G(x) = F_{\alpha}(x)$.
  Clearly $\alpha < \beta$ implies 
  $F(\{ x_{\alpha}, x_{\beta}\}) = F_{x_{\beta}}(x_{\alpha}) = F_a(x_{\alpha}) = G_{x_{\alpha}}$.
  The range of $G$ is countable, so there is $H \subset X$ of size $\aleph_1$ such that $G$
  is constant on $H$. Thus $F$ is constant on $[H]^2$.

  So we have proved the theorem for $n = 1$. Let us assume that 
  $\beta^+_{n - 1} \to (\aleph_1)^n_{\aleph_0}$ and let $F : [\kappa]^{n + 1} \to \omega$
  where $\kappa = \beth^+_n$. For each $a \in \kappa$, let $F_{a} : [\kappa - \{ a\}]^n \to \omega$
  be defined by such that for every $C \subset A$ of size $|C| \leq \beth_{n - 1}$
  and every $u \in \kappa - C$ there exists $v \in A - C$ such that $F_v$ agrees with $F_u$ on $[C]^n$.

  We choose $a \in \kappa - A$ and construct a set $X = \{ x_{\alpha} \: | \: \alpha < \beth^+_{n - 1}\} \subset A$
  such that for each $\alpha$, $F_{x_{\alpha}}$ agrees with $F_{a}$ on $[\{ x_{\beta} \: | \: \beta < \alpha \}]^n$.

  Consider $G : [X]^n \to \omega$ where $G(x) = F_a(x)$. If $\alpha_1 < \ldots < \alpha_{n + 1}$,
  then $F(\{x_{{\alpha}_1}, \ldots, F_{{\alpha_{n + 1}}}\}) = G(\{x_{{\alpha}_1}, \ldots, F_{{\alpha_{n}}}\})$.
  By the induction hypothesis, there is $H \subset X$ of size $\aleph_1$ such that $G$ is constant on $[H]^n$.
  Thus $F$ is constant on $[H]^{n + 1}$.
\end{proof}
